...
void GarfieldDetectorConstruction::DefineMaterials() {
	...
	G4Material* CF4 = new G4Material("CF4",
			density = 3.72 * CLHEP::mg / CLHEP::cm3, ncomponents = 2);
	...
	NeCF4_95_5->AddMaterial(CF4, fractionmass = 0.05);
...
G4VPhysicalVolume* GarfieldDetectorConstruction::DefineVolumes() {
	...
	*/
	...
	fTubePVO =  new G4PVPlacement(0,G4ThreeVector(0.,0.,0.), // its position
	...
	G4VSolid* gasS = new G4Tubs("Gas",            // its name
			tubeInnerRadius, tubeRadius - tubeThickness, tubeHalfLength, 0, 2 * pi); // its size
