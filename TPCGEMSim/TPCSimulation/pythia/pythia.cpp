...
#endif
{
  ...
  if (argc != 3) {
    ...
    const Int_t clonesarraysize = 5000; // size of TClonesArray to store
                                        // generated particles
    ...
    //eg->SetCKIN(3,5);  // lower cut off on p_T 
    //eg->SetCKIN(15,4.6); //cut on pseudo-rapidity
    //eg->SetCKIN(28,0.999);
    ...
    // Switch on all processes 
    eg->SetMSUB(1, 1);
    //decay only in t tbar
